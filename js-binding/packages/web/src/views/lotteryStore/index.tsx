...
export interface LotteryState {
  ...
  // number of editions for this lottery (only applicable to limited edition)
  editions?: number;

  // date time when lottery should start UTC+0
  startDate?: Date;

  // suggested date time when lottery should end UTC+0
  endDate?: Date;

  //////////////////
  category: LotteryCategory;
  saleType?: 'lottery' | 'sale';
  ...
  lotteryDurationType?: 'days' | 'hours' | 'minutes';
  gapTime?: number;
...
export const LotteryCreateView = () => {
  ...
  const [lotteryObj, setLotteryObj] =
    useState<
      | {
          vault: StringPublicKey;
          lottery: StringPublicKey;
          lotteryManager: StringPublicKey;
        }
      | undefined
    >(undefined);
  const [attributes, setAttributes] = useState<LotteryState>({
    reservationPrice: 0,
    items: [],
    category: LotteryCategory.Open,
    saleType: 'lottery',
    lotteryDurationType: 'minutes',
    gapTimeType: 'minutes',
    winnersCount: 1,
  ...
  const [tieredAttributes, setTieredAttributes] = useState<TieredLotteryState>({
    items: [],
    tiers: [],
  });
  ...
  const gotoNextStep = (_step?: number) => {
    ...
    history.push(`/lottery/create/${nextStep.toString()}`);
  };

  const createLottery = async () => {
    let winnerLimit: WinnerLimit;
    if (attributes.category === LotteryCategory.Open) {
      if (
      ...
      attributes.category === LotteryCategory.Limited ||
      attributes.category === LotteryCategory.Single
    ) {
    ...
    // const lotterySettings: IPartialCreateAuctionArgs = {
    //   winners: winnerLimit,
    //   endLotteryAt: new BN(
    //     (attributes.lotteryDuration || 0) *
    //       (attributes.lotteryDurationType == 'days'
    //         ? 60 * 60 * 24 // 1 day in seconds
    //         : attributes.lotteryDurationType == 'hours'
    //         ? 60 * 60 // 1 hour in seconds
    //         : 60), // 1 minute in seconds
    //   ), // endLotteryAt is actually lottery duration, poorly named, in seconds
    //   lotteryGap: new BN(
    ...
    // const _lotteryObj = await createAuctionManager(
    //   connection,
    //   wallet,
    //   whitelistedCreatorsByCreator,
    //   lotterySettings,
    //   attributes.category === LotteryCategory.Open
    //     ? []
    //     : attributes.category !== LotteryCategory.Tiered
    //     ? attributes.items
    ...
    //   QUOTE_MINT.toBase58(),
    // );
    // setLotteryObj(_lotteryObj);
  };

  const categoryStep = (
    <CategoryStep
      confirm={(category: LotteryCategory) => {
        setAttributes({
          ...attributes,
          category,
        });
  ...
  const waitStep = (
    <WaitingStep createLottery={createLottery} confirm={() => gotoNextStep()} />
  );

  const congratsStep = <Congrats lottery={lotteryObj} />;

  const stepsByCategory = {
    [LotteryCategory.Limited]: [
...
const CategoryStep = (props: {
  confirm: (category: LotteryCategory) => void;
}) => {
  const { width } = useWindowDimensions();
  return (
    <>
        ...
        <Col>
          <Row>
            <Button
              className="type-btn"
              size="large"
              onClick={() => props.confirm(LotteryCategory.Limited)}
            >
              <div>
          ...
          <Row>
            <Button
              className="type-btn"
              size="large"
              onClick={() => props.confirm(LotteryCategory.Open)}
            >
              <div>
          ...
          <Row>
            <Button
              className="type-btn"
              size="large"
              onClick={() => props.confirm(LotteryCategory.Tiered)}
            >
              <div>
                <div>Tiered Lottery</div>
                <div className="type-btn-description">
          ...
          <Row>
            <Button
              className="type-btn"
              size="large"
              onClick={() => props.confirm(LotteryCategory.Single)}
            >
              <div>
...
const CopiesStep = (props: {
  attributes: LotteryState;
  setAttributes: (attr: LotteryState) => void;
  confirm: () => void;
}) => {
  ...
  let filter: (i: SafetyDepositDraft) => boolean = (i: SafetyDepositDraft) =>
    true;
  if (props.attributes.category === LotteryCategory.Limited) {
    filter = (i: SafetyDepositDraft) =>
      !!i.masterEdition && !!i.masterEdition.info.maxSupply;
  } else if (props.attributes.category === LotteryCategory.Open) {
    filter = (i: SafetyDepositDraft) =>
      !!(
        i.masterEdition &&
    ...
    <>
      ...
      <Row className="content-action">
        <Col xl={24}>
          ...
          {props.attributes.category === LotteryCategory.Limited && (
...
const NumberOfWinnersStep = (props: {
  ...
  return (
    <>
      <Row className="call-to-action">
        <h2>Tiered Lottery</h2>
        <p>Create a Tiered Lottery</p>
      </Row>
      <Row className="content-action">
        <Col className="section" xl={24}>
          <label className="action-field">
            <span className="field-title">
              How many participants can win the lottery?
            </span>
            <span className="field-info">
      ...
      <Row className="content-action">
        <Col className="section" xl={24}>
          <label className="action-field">
            ...
            >
              <Radio className="radio-field" value="lottery">
                Lottery
              </Radio>
...
const PriceStep = (props: {
  ...
  return (
    <>
      {props.attributes.saleType === 'lottery' ? (
        <PriceLottery {...props} />
      ) : (
        <PriceSale {...props} />
      )}
...
const PriceSale = (props: {
  ...
  return (
    <>
      <Row className="call-to-action">
        <h2>Price</h2>
        <p>Set the price for your lottery.</p>
      </Row>
      <Row className="content-action">
        <label className="action-field">
          <span className="field-title">Sale price</span>
          <span className="field-info">
            This is the starting bid price for your lottery.
          </span>
          <Input
            type="number"
            min={0}
...
const PriceLottery = (props: {
  ...
  return (
    <>
      ...
      <Row className="content-action">
        <Col className="section" xl={24}>
          {props.attributes.category === LotteryCategory.Open && (
            <label className="action-field">
...
const EndingPhaseLottery = (props: {
  attributes: LotteryState;
  setAttributes: (attr: LotteryState) => void;
  confirm: () => void;
}) => {
  return (
    <>
      <Row className="call-to-action">
        <h2>Ending Phase</h2>
        <p>Set the terms for your lottery.</p>
      </Row>
      <Row className="content-action">
        <Col className="section" xl={24}>
          <div className="action-field">
            <span className="field-title">Lottery Duration</span>
            <span className="field-info">
              This is how long the lottery will last for.
            </span>
            <Input
              addonAfter={
                <Select
                  defaultValue={props.attributes.lotteryDurationType}
                  onChange={value =>
                    props.setAttributes({
                      ...props.attributes,
                      lotteryDurationType: value,
                    })
                  }
                >
              ...
              type="number"
              className="input"
              placeholder="Set the lottery duration"
              onChange={info =>
                props.setAttributes({
                  ...props.attributes,
                  lotteryDuration: parseInt(info.target.value),
                })
              }
            />
          </div>
          ...
          <div className="action-field">
            ...
            <span className="field-info">
              The final phase of the lottery will begin when there is this much
          ...
          <label className="action-field">
            ...
            <span className="field-info">
              In order for winners to move up in the lottery, they must place a
...
const TierTableStep = (props: {
  attributes: TieredLotteryState;
  setAttributes: (attr: TieredLotteryState) => void;
  maxWinners: number;
  confirm: () => void;
}) => {
  ...
  return (
    <>
      <Row className="call-to-action">
        ...
        <p>
          Provide NFT that will be awarded as an Open Edition NFT for lottery
          participation.
        </p>
      </Row>
      <Row className="content-action">
...
const Congrats = (props: {
  ...
  const newTweetURL = () => {
    const params = {
      text: "I've created a new NFT lottery on Metaplex, check it out!",
      url: `${
        window.location.origin
      }/#/lottery/${props.lottery?.lottery.toString()}`,
  ...
  return (
    <>
      ...
      >
        <div className="waiting-title">
          Congratulations! Your lottery is now live.
        </div>
        <div className="congrats-button-container">
          ...
          <Button
            className="metaplex-button"
            onClick={_ =>
              history.push(`/lottery/${props.lottery?.lottery.toString()}`)
            }
          >
            <span>See it in your lotterys</span>
            <span>&gt;</span>
          </Button>
        </div>
      </div>
